...
<title>{{ $product->name_en }} — Sri Vayaluran Herbals</title>
...
<header>
  <div class="container">
    <nav>
      <a href="{{ route('home') }}" class="logo"><i class="fa-solid fa-leaf"></i> Sri Vayaluran Herbals</a>
      <div class="nav-right">
        <a href="{{ route('home') }}#products" class="back-link"><i class="fa-solid fa-arrow-left"></i> Back to Shop</a>
...
<div class="container breadcrumb">
  ...
  <a href="{{ route('home') }}#products">{{ $product->category->name_en ?? 'Shop' }}</a> /
    ...
    <div class="product-hero-grid">
      <div class="pd-visual">
        <div class="pd-image-frame">
          ...
          @else
            {!! \App\Support\ProductDisplay::visual($product, 260) !!}
          @endif
        </div>
      </div>

      <div class="pd-info">
        <div class="pd-cat">{{ $product->category->name_en ?? 'Sri Vayaluran Herbals' }}</div>
        <h1 class="pd-title">{{ $product->name_en }}</h1>
        @if ($product->name_ta)<div class="pd-title-ta">{{ $product->name_ta }}</div>@endif
        ...
        @if ($product->variants->isNotEmpty())
          ...
          <div class="pack-options" id="packOptions">
            @foreach ($product->variants as $i => $variant)
              <button type="button" class="pack-option {{ $i === 0 ? 'active' : '' }}"
                      data-variant-id="{{ $variant->id }}"
                      data-price="{{ \App\Support\ProductDisplay::money($variant->price) }}">
                <span class="pk-size">{{ \App\Support\ProductDisplay::money($variant->value) }} {{ $variant->unit }}</span>
                <span class="pk-price">₹{{ \App\Support\ProductDisplay::money($variant->price) }}</span>
              </button>
            @endforeach
          </div>
          ...
          <div class="pd-buy-row">
            <div class="pd-price-display">
              ...
              <span id="selectedPrice">₹{{ \App\Support\ProductDisplay::money($product->variants->first()->price) }}</span>
            </div>
        ...
        @if ($product->additional_details)
          <div class="pd-accordion">
            <details open>
              ...
              <div class="pd-accordion-body">{{ $product->additional_details }}</div>
            </details>
          </div>
        @endif
      </div>
    ...
    <div class="container">
      ...
      <div class="related-grid">
        @foreach ($related as $rp)
          <a href="{{ route('product.show', $rp) }}" class="related-card">
            <div class="rc-jar">{!! \App\Support\ProductDisplay::visual($rp, 88) !!}</div>
            <h4>{{ $rp->name_en }}</h4>
            @if ($rp->name_ta)<div style="font-size:.74rem;color:#8a7f6a;">{{ $rp->name_ta }}</div>@endif
            <div class="rc-price">{!! \App\Support\ProductDisplay::priceSummary($rp) !!}</div>
          </a>
        @endforeach
      </div>
    </div>
...
<footer>
  <div class="container">
    &copy; {{ date('Y') }} Sri Vayaluran Herbals. All remedies prepared with care, since 1918. ·
...
let selectedPrice = {{ \App\Support\ProductDisplay::money($product->variants->first()->price ?? 0) }};
