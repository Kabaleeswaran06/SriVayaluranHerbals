...
<title>Shop — Sri Vayaluran Herbals</title>
...
<link rel="icon" type="image/jpeg" href="{{ asset('images/logoa.jpeg') }}">
<style>
:root{
  --forest:#16321F;
  --forest-2:#1F4028;
...
<header class="shop-header">
  ...
  <div class="container">
    <nav class="shop-nav">
      <a href="{{ url('/') }}" class="shop-logo">
        <img src="{{ asset('images/logoa.jpeg') }}" alt="Sri Vayaluran Herbals">
  ...
  <div class="container">
    ...
    @else
      <div class="count-strip reveal">
        <span class="count-pill">{{ $categories->count() }} {{ Str::plural('Category', $categories->count()) }} Available</span>
      </div>

      <div class="cat-grid">
        @foreach($categories as $category)
          <a href="{{ route('shop.category', $category) }}" class="cat-card reveal-item">
            <div class="cat-img">
              @if($category->image_url ?? $category->image ?? false)
                <img src="{{ $category->image_url ?? asset('storage/'.$category->image) }}" alt="{{ $category->name_en }}">
              @else
                🌿
              @endif
            ...
            <div class="cat-body">
              <p class="cat-name-en">{{ $category->name_en }}</p>
              @if($category->name_ta)
                <p class="cat-name-ta">{{ $category->name_ta }}</p>
              @endif
              <span class="cat-count"><i class="fa-solid fa-mortar-pestle"></i> {{ $category->products_count }} {{ Str::plural('product', $category->products_count) }}</span>
            </div>
          </a>
        @endforeach
      </div>
