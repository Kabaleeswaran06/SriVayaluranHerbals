...
/**
 * Small display helpers shared across the public product views
 * (home, product-show, cart). Static methods instead of Blade-local
 * closures so they work no matter which view calls them.
 */
class ProductDisplay
{
    private static array $jarColors = ['#3F6B44', '#7A2E2E', '#C9A227', '#6E9B6E', '#E08A1E', '#E4C860'];

    public static function visual(Product $product, int $size): string
    {
        ...
        $color = self::$jarColors[$product->id % count(self::$jarColors)];
    ...
    public static function priceSummary(Product $product): string
    {
        ...
        if ($variants->count() === 1) {
            return '₹'.self::money($variants[0]->price);
        }
        $min = $variants->min('price');

        return '<span class="price-from">From</span>₹'.self::money($min);
    }
